...
post_configure() {
	...
	sed -i.orig -e "s/^\(SUBDIRS = .\+ \)examples tests /\1/" Makefile &&
	# operations 以下のビルドで bebl をリンクしない問題の修正。
	sed -i.orig -e 's/^LIBS =\s*\(\|$(op_libs)\)\s*$/\0 $(BABL_LIBS)/' operations/{affine,core,common,generated,external}/Makefile
...
run_make() {
	${XMINGW}/cross make SHREXT=".dll" libgegl="-L${PWD}/gegl/.libs -lgegl-0.1" install
}
...
run_pack_archive() {
	cd "${INSTALL_TARGET}" &&
	pack_archive "${BINZIP}" bin/*.dll lib/gegl-0.1/*.dll &&
	pack_archive "${DEVZIP}" include lib/*.a lib/gegl-0.1/*.a lib/pkgconfig &&
